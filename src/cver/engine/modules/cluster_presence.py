...
class ClusterPresence:
    ...
    def run(self):
        logger.info("Starting Cluster Presence checks.")
        logger.info("Cluster presence interval hours: %s" % self.cluster_presence_hours)
    ...
    def handel_cluster_images(self, cluster):
        ...
        cluster_images = col_ci.get_all(request_args)
        logger.info("Found %s CluserImages for cluster %s" % (len(cluster_images), cluster))
    ...
    def handel_cluster_image_builds(self, cluster):
        ...
        logger.info("Found %s CluserImageBuilds for cluster %s" % (
        ...
        for cluster_image_build in cluster_image_builds:
            ...
            if interval_ready:
                ...
                logger.info("Setting %s to no longer present in cluster %s" % (

"""Regression Test CTRL Vendors
Checks that all routes on /vendors are working properly.
...
class TestApiVendors:

    def test__vendors_get(self):
        """Tests the Vendors collections through the Cver Api
        GET /vendors
        """
        request_args = {
            "headers": HEADERS,
            "method": "GET",
            "url": "%s/vendors" % CVER_API_URL,
        }
    ...
    def test__vendors_get_pagination(self):
        """Tests the Vendors collections through the Vendors Api
        GET /vendors
        """
        request_args = {
            "headers": HEADERS,
        ...
        assert response.status_code == 200
        response_json = response.json()
        assert response_json["info"]["current_page"] == 5
